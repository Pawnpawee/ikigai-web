...
const OPTIONS: { value: EverPaidAnswer; label: string }[] = [
  { value: "yes", label: "ใช่" }, { value: "no", label: "ไม่" },
];

export default function Scene1EverPaid({ opacity, zIndex, textProgress, everPaidAnswer, setEverPaidAnswer, errorMessage }: Props) {
  const text = "แมว : เจ้ามาถึงตลาดแห่งรายได้แล้ว… สถานที่ที่ทุกทักษะ ทุกความสามารถ มีค่าและสามารถเปลี่ยนเป็นรายได้ได้ เจ้าล่ะ เคยได้รับค่าจ้างจากสิ่งใดบ้างหรือไม่?";
...
className="typo-h5 text-white" />
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex gap-4">
          {OPTIONS.map((opt) => (
            <motion.button key={opt.value} onClick={() => setEverPaidAnswer(opt.value)}
