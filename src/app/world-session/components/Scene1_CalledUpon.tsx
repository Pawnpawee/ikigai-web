...
export default function Scene1CalledUpon({
  opacity,
  zIndex,
  textProgress,
  calledUponAnswer,
  ...
  const text =
    "แมว : เจ้ามองเห็นหรือไม่… โลกใบนี้เต็มไปด้วยเสียงเรียกร้อง ทุกแห่งต่างตามหาผู้คนที่จะมอบบางสิ่งให้แก่พวกเขา เจ้าล่ะ… เคยมีใครเรียกร้องให้เจ้าช่วยเหลือสิ่งใดบ้างไหม หรือลองคิดถึงครั้งล่าสุดที่มีคนชมผลงานของเจ้า ?";
      ...
      <div className="flex flex-col items-center gap-6 sm:gap-8 p-3 sm:p-4 max-w-4xl text-center">
        ...
        >
          {OPTIONS.map((opt) => (
            <motion.button
              key={opt.value}
              onClick={() => setCalledUponAnswer(opt.value)}
