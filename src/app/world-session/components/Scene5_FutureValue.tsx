...
export default function Scene5FutureValue({ opacity, zIndex, textProgress, futureValueAnswer, setFutureValueAnswer, errorMessage, handleSubmit, isLoading }: Props) {
...
className="typo-h5 text-white" />
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex gap-4">
          {OPTIONS.map((opt) => (
            <motion.button key={opt.value} onClick={() => setFutureValueAnswer(opt.value)}
