...
export default function Scene4Mismatch({
  opacity,
  zIndex,
  textProgress,
  mismatchChoice,
  ...
  const text =
    "ถ้างานที่ได้รับไม่ตรงกับสิ่งที่เจ้าอยากทำ เช่น งานด้านตัวเลขในขณะที่เจ้าใฝ่หาความคิดสร้างสรรค์… เจ้าจะเลือกอย่างไร";
      ...
      <div className="flex flex-col items-center gap-6 sm:gap-8 p-3 sm:p-4 max-w-3xl text-center">
        ...
        >
          {OPTIONS.map((opt) => (
            <motion.button
              key={opt.value}
              onClick={() => setMismatchChoice(opt.value)}
